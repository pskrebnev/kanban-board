...
try {
  ...
  const editInput = page.locator(".team-edit input");
  await editInput.fill(renamedTeam);
  await page.getByRole("button", { name: "Save" }).click();
  await page.getByText(renamedTeam, { exact: true }).waitFor({ timeout: 20_000 });

  // Delete it (unreferenced, so deletion succeeds).
  await page.getByRole("button", { name: "Delete" }).first().click();
  await page.getByRole("button", { name: "Confirm" }).click();
  await page.getByText(/no teams yet/i).waitFor({ timeout: 20_000 });
